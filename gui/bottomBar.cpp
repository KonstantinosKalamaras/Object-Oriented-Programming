...
void BottomBar::rollDice()
{
    ...
    {
        Card* card = myWindow->getNextCommunityCard();
        monopolyBoard->showCommunity(card->getId());
        space = boardGame->actOnCard(card);
        myWindow->setPlayerLocation(currentPlayerNum, space->getId());
    }
    else if(space->typeIs(ChanceType))
    {
        Card* card = myWindow->getNextChanceCard();
        monopolyBoard->showChance(card->getId());
        space = boardGame->actOnCard(card);
        myWindow->setPlayerLocation(currentPlayerNum, space->getId());
    }
...
bool BottomBar::upgradeProperty(int propertyToUpgrade)
{
    ...
    if (space->typeIs(PropertyType) && myWindow->getCurrentPlayer()->canUpgrade((Property*)space))
    {
        ...
        myWindow->getCurrentPlayer()->takeMoneyFromPlayer(((Property*)boardGame->getSpace(propertyToUpgrade))->getUpgradeCost());
        myWindow->getBank()->giveMoneyToBank(((Property*)boardGame->getSpace(propertyToUpgrade))->getUpgradeCost());
