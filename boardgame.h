...
public:
     ...
     * @param community the deck of community cards.
     * @param chance the deck of chance cards.
     */
    BoardGame(Player** players, Bank* bank, Deck* community, Deck* chance);
    ...
    /**
     * Function that makes an action for a player according to the card drawn. Different actions take place
     * according to the instructions of the card. The function also returns the resulting space where the player
     * is moved (used for actions that move the player, such as go to some space)
     *
     * @param card the card that the player has drawn.
     * @return the resulting space where the player is moved.
     */
    Space* actOnCard(Card* card);
